...
export const SessionRoster: React.FC<SessionRosterProps> = ({ students, onStatusChange }) => {
    ...
    <div className="space-y-8">
      ...
      <section>
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          ...
          <button
            onClick={() => alert(`Roster submitted!\nPresent: ${presentCount} | Absent: ${absentCount} | Late: ${lateCount}`)}
            className="bg-primary text-white font-headline font-bold px-8 py-3 rounded-xl shadow-lg hover:shadow-primary/20 transition-all active:scale-95 flex items-center gap-2 bg-gradient-to-r from-primary to-primary-container"
          >
          ...
          <div key={student.id} className="group bg-surface-container-lowest p-4 rounded-2xl flex items-center justify-between transition-all hover:bg-surface-container-high border border-transparent hover:border-outline-variant/10">
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`px-4 py-2 rounded-full font-label text-[11px] font-bold tracking-wider uppercase transition-all active:scale-95 ${
